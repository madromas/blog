...
$errors = [];
$title = $content = $tags = $existingImage = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title)) {
        ...
        if (empty($errors)) {
        if ($action == 'create') {
            if (createPost($title, $content, $image, $tags)) {
                $_SESSION['message'] = "Post created successfully.";
                $_SESSION['message_type'] = 'success';
                header("Location: admin.php?tab=posts&message=Post created successfully.");
                exit;
            } else {
            ...
            if (updatePost($post_id, $title, $content, $image, $tags)) {
                ...
                $_SESSION['message_type'] = 'success';
                header("Location: admin.php?tab=posts&message=Post updated successfully.");
                exit;
            } else {
...
$page_title = ($action == 'edit') ? 'Edit Post' : 'Create Post';
...
<div class="create-post-container">
    <h1 class="page-title">
        ...
        <?= ($action == 'edit') ? 'Edit Post' : 'Create New Post' ?>
    </h1>
    ...
    <form action="admin_post_form.php?action=<?= $action ?><?= ($action == 'edit') ? '&id=' . $post_id : '' ?>" method="POST" enctype="multipart/form-data" class="post-form">
...
<?php
// We don't need to include header/footer in admin section, because it already included in admin.php
?>